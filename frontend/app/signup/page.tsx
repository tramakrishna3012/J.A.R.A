...
export default function SignupPage() {
    ...
    const handleEmailAuth = async (e: React.FormEvent) => {
        ...
        if (error) {
            ...
            // Trigger animation sequence to simulate setup
            setStep('verify');
            setTimeout(() => {
                setVerifying(true);
                setTimeout(() => {
                ...
                {step === 'login' && (
                    <div className="w-full max-w-[440px] glass-card shadow-2xl rounded-3xl p-10 relative overflow-hidden animate-fade-in-up">
                        <div className="mb-10 text-center">
                            <h1 className="text-3xl font-bold mb-3 tracking-tight">Request Clearance</h1>
                            <p className="text-ash text-sm">Create an account to deploy your autonomous agent.</p>
                        </div>
                        ...
                        <form onSubmit={handleEmailAuth} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-[10px] font-bold text-ash uppercase tracking-widest mb-2 ml-1">Create Password</label>
                                <input 
                                    type="password" 
                                    required
                                    value={password}
                            ...
                            >
                                {!loading ? (
                                    <span>Generate Clearance Codes</span>
                                ) : (
                        ...
                        <p className="mt-8 text-center text-xs text-ash">
                            Already cleared? <Link href="/login" className="text-electric hover:text-neon transition">Access Terminal</Link>
                        </p>
                    </div>
                )}
                ...
                {step === 'verify' && (
                    <div className="w-full max-w-[500px] text-center animate-fade-in-up">
                        ...
                        <div className="space-y-6">
                            <h2 className="text-3xl font-bold italic tracking-tighter">Registering Neural Identity...</h2>
                            ...
                            <div className="bg-electric/10 border border-electric/30 border-dashed p-6 rounded-2xl max-w-sm mx-auto font-mono text-left">
                                <div className="flex justify-between text-[10px] text-electric mb-2">
                                    <span>AGENT_INITIALIZATION</span>
                                    <span className="animate-pulse">ONLINE</span>
                                </div>
                                <div className="space-y-1 text-xs">
                                    <div className="flex justify-between">
                                        <span className="text-ash">SUBJECT:</span>
                                        <span>{email || 'NEW_USER_ALPHA'}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-ash">CLEARANCE:</span>
                                        <span>PENDING_EMAIL_VERIFICATION</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-ash">STATUS:</span>
                                        <span className="text-neon">GENERATING KEYS</span>
                                    </div>
                                </div>
                            </div>
