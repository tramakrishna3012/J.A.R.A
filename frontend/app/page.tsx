...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col bg-night text-slate-200 selection:bg-electric/30 overflow-hidden relative">

      {/* Background Gradients */}
      <div className="absolute inset-0 premium-gradient pointer-events-none -z-10 animate-pulse-slow" />

      {/* Navbar */}
      <nav className="w-full max-w-7xl mx-auto flex items-center justify-between p-6 z-50">
        <div className="flex items-center gap-3">
          <img src="/logo.png" alt="J.A.R.A Logo" className="h-10 w-auto" />
      ...
      <section className="flex flex-col items-center text-center max-w-5xl mx-auto mt-20 px-4 z-10">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-neon/30 bg-neon/10 text-neon text-sm font-medium mb-8 animate-fade-in-up">
        ...
        <h1 className="text-6xl md:text-8xl font-extrabold tracking-tight mb-8 leading-tight bg-clip-text text-transparent bg-gradient-to-b from-white via-white to-gray-400">
          ...
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-electric to-neon">On Autopilot</span>
        </h1>
        ...
        <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
          <Link href="/dashboard" className="group flex items-center justify-center gap-3 px-8 py-4 bg-electric rounded-full font-bold text-lg hover:bg-electric/90 text-white transition shadow-xl shadow-electric/20">
        ...
        {/* Floating UI Mockup */}
        <div className="mt-20 relative w-full max-w-4xl mx-auto rounded-xl border border-white/10 bg-obsidian/50 backdrop-blur-xl shadow-2xl p-4 transform rotate-x-12 perspective-1000 animate-float">
          <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent z-20" />
          <div className="flex items-center gap-2 px-4 py-3 border-b border-white/10 bg-obsidian/80 rounded-t-lg">
        ...
        {[
          ...
          <div key={i} className="group p-8 rounded-3xl border border-white/10 bg-zinc-900/30 hover:bg-zinc-800/50 transition hover:-translate-y-1 duration-300">
            <div className="w-14 h-14 rounded-2xl bg-zinc-800 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
              <f.icon className="w-7 h-7 text-electric" />
            </div>
      ...
      {/* Footer */}
      <footer className="w-full border-t border-white/10 py-12 bg-obsidian">
